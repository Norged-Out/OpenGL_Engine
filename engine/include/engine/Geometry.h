...
public:
    ...
    static std::unique_ptr<Mesh> createSphereMesh(int stacks = 16, int slices = 24);

    // -------- High-level helpers --------

    // Creates a sphere and optionally attaches textures
    // If applyTextures == false, returns a pure geometry mesh
    static std::unique_ptr<Mesh> createTexturedSphere(bool applyTextures, 
        const std::string& texPath = "", int stacks = 16, int slices = 24);

    // -------- Tangent utilities --------

    // Generic tangent generator (reusable for any indexed mesh)
    static void generateTangents(std::vector<Vertex>& vertices, const std::vector<GLuint>& indices);
};